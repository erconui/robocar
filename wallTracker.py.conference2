...
class OpticFlowControl():
    ...
    def __init__(self, img_height, img_width, of_res = 20):
        ...
        self.l_center = np.array([50, 2*img_height//3])
        self.r_center = np.array([img_width - 50, 2*img_height//3])
        self.r_width = 100
        self.r_height = 200
        self.l_width = 100
        self.l_height = 200
        self.c_width = 100#img_width - 100
        self.c_height = 100#img_height -100
        self.d_center = np.array([img_width//2, img_height - 50])
        self.d_width = 300
        self.d_height = 100
        ...
        self.down_mask, self.draw_down = self.getMask(self.d_center, self.d_height, self.d_width, (255,0,0))
    ...
    def annotate(self, img):
        ...
        self.addArrows(mask, 10, 10, self.d_center, self.d_width, self.d_height, self.d_avg, (255,0,255))
        ...
        for i in range(-num_height//2,num_height//2):
            for j in range(-num_width//2,num_width//2):
                ...
                cv2.arrowedLine(
                    ...
                    color, 1)
    def getP0(self, center, width, height, num_points):
        point_range = range(-num_points//2, num_points//2)
        return np.array([[
            [
                center[0] + x*width//num_points,
                center[1] + j*height//num_points] for x in point_range for j in point_range
            ]], dtype=np.float32)
    ...
    def calc_optic_flow(self, img):
        ...
        num_points = 100
        point_range = range(-num_points//2, num_points//2)
        self.p_right = self.getP0(
            self.r_center, self.r_width, self.r_height, num_points)
        self.p_left = self.getP0(
            self.l_center, self.l_width, self.l_height, num_points)
        self.p_center = self.getP0(
            self.c_center, self.c_width, self.l_height, num_points)
        self.p_down = self.getP0(
            self.d_center, self.d_width, self.d_height, num_points)
